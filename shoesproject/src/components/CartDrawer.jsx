...
export const CartDrawer = () => {
  ...
  const handleCheckout = () => {
    ...
    // Очищаем корзину (поддерживаем разные варианты написания функции в хуке)
    if (typeof clearCart === 'function') {
      clearCart();
    } else if (typeof setCartItems === 'function') {
      setCartItems([]);
    } else {
      // Запасной вариант: удаляем каждый товар по очереди
      cartItems.forEach(item => removeFromCart(item.id, item.size, item.color));
    }

    // Через 3 секунды закрываем корзину и возвращаем обычный вид
    setTimeout(() => {
      setIsSuccess(false);
      setIsCartOpen(false);
    }, 3000);
    ...
    <AnimatePresence>
      {isCartOpen && (
        <>
          ...
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed right-0 top-0 h-full w-full max-w-md bg-white z-50 shadow-2xl flex flex-col">
            ...
            {!isSuccess && cartItems.length > 0 && (
              <div className="border-t p-6 space-y-4">
                ...
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleCheckout} className="w-full bg-gray-900 text-white py-4 rounded-xl font-bold hover:bg-gray-800 transition-colors">Sifariş Et</motion.button>
