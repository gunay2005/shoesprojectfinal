...
export const ProductCard = ({ product, displayImageIndex = 0 }) => {
  ...
  // Məhsulun favoritlərdə olub-olmadığını yoxlayırıq
  const isWishlisted = favorites.some((fav) => fav.id === product.id);

  // Əsas şəkil: əgər displayImageIndex verilibsə — images[index], əks halda image və ya images[0]
  const mainImage = product.images?.[displayImageIndex] || product.image || product.images?.[0];

  // hoverImage — hover effekti üçün digər şəkil
  ...
  const handleCardClick = () => {
    // Səhifə nömrəsi və digər query parametrləri (page, category və s.) məhsul səhifəsinə ötürülür,
    // ki, "Mağazaya qayıt" düyməsi düzgün səhifəyə qaytarsın
    ...
    >
      ...
      >
        ...
        <img 
          src={mainImage} 
          alt={product.name} 
          className={`absolute inset-0 w-full h-full object-cover transition-all duration-700 ease-out ${isHovered && hasHoverImage ? 'opacity-0 scale-105' : 'opacity-100 scale-100'}`}
        />
        ...
        {hasHoverImage && (
          <img 
            ...
            className={`absolute inset-0 w-full h-full object-cover transition-all duration-700 ease-out ${isHovered ? 'opacity-100 scale-100' : 'opacity-0 scale-105'}`}
          />
        )}
          ...
          {product.badge && (
            <motion.span 
              ...
              className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase text-white ${
                product.badge === 'Endirim' ? 'bg-red-500' : 
                product.badge === 'Yeni' ? 'bg-emerald-500' : 'bg-rose-500'
              }`}
            >
              {product.badge}
            </motion.span>
        ...
        >
          <Heart size={15} className={isWishlisted ? 'fill-rose-500 text-rose-500' : 'text-gray-600'} />
        </motion.button>
      ...
      <div className="mt-3.5 px-0.5">
        <div className="flex items-center gap-1 mb-1">
          ...
          <span className="text-[11px] text-gray-400 ml-1">({product.reviews || 0})</span>
        </div>
        <h3 
